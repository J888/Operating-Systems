...
int write_metadata(int type, int firstFATblock, char * filename, char * extension, int filesize)
{
	...
	//write the metadata	
	fwrite(filename, 8, 1, drive_fp);      //filename
	fseek(drive_fp, 8, SEEK_CUR);   	
	fwrite(extension, 3, 1, drive_fp); 	//extension
	fseek(drive_fp, 3, SEEK_CUR); 			
	fwrite(date, 8, 1, drive_fp);			//date created
	fseek(drive_fp, 8, SEEK_CUR);			
	fwrite(date, 8, 1, drive_fp);			//date modified
	fseek(drive_fp, 8, SEEK_CUR);
	fwrite(firstFATblock, 2, 1, drive_fp); //first fat block
	fseek(drive_fp, 2, SEEK_CUR); 			
	fwrite(filesize, 2, 1, drive_fp);		//filesize
	fseek(drive_fp, 2, SEEK_CUR);
	fwrite(type, 1, 1, drive_fp);			//filetype
